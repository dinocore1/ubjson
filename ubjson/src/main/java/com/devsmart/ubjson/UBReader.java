...
public class UBReader implements Closeable {
    ...
    private long readInt64() throws IOException {
        return ((long)readControl() & 0xFF) << 56 | ((long)readControl() & 0xFF) << 48
    ...
    private byte[] readString(byte control) throws IOException {
        ...
        int offset = 0;
        while(bytesLeft > 0) {
            int bytesRead = mInputStream.read(value, offset, bytesLeft);
            if(bytesRead < 0) {
    ...
    private UBArray readArray() throws IOException {
        ...
        if(control == UBValue.MARKER_OPTIMIZED_TYPE) {
            ...
            switch (type) {
                ...
                case UBValue.MARKER_INT64:
                    return UBValueFactory.createArray(readOptimizedArrayInt64(size));
